...
LOGIN_PER_IP = "10/minute"        # 登录限流，单 IP 每分钟最多 10 次
REGISTER_PER_IP = "10/minute"     # 注册限流，单 IP 每分钟最多 10 次
USER_ME_PER_USER = "60/minute"    # 个人信息限流，单用户每分钟最多 60 次

limiter = Limiter(
    key_func=get_client_ip_for_rate_limit,
    # 全局默认不限流，仅在挂了 @limiter.limit(...) 的路由上生效。
    default_limits=[],
)
...
def get_user_rate_limit_key(request: Request) -> str:
    ...
    Returns:
        str: 格式为 user:用户 ID，无令牌或解码失败时退回 ip:客户端 IP。
    """
